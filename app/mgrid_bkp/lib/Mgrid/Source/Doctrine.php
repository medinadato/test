 ...
 */
class Doctrine extends \Mgrid\Source\ASource implements \Mgrid\Source\ISource
{
    /**
     * @var QueryBuilder
     */
    ...
    {
        $this->setQueryBuilder($source);
    }
    
    public function count()
    {
	return $q = $this->getNumResults();
    }
    ...
    public function setEntityManager($entityManager = null)
    {
        if(is_null($entityManager)) {
            if(\Zend_Registry::isRegistered('doctrine')) {
                ...
                throw new Mgrid_Grid_Source_Doctrine2_Exception('No suitable EntityManager found in registry, please set a specific one.');
            }
        } elseif(!($entityManager instanceof EntityManager)) {
            throw new Mgrid_Grid_Source_Doctrine2_Exception('Parameter must be an instance of \Doctrine\ORM\EntityManager');
        }
     ...
     * @return Mgrid_Grid_Source_Doctrine2
     */
    public function setQueryBuilder($value)
    {
        ...
        if(is_string($value)) {
            ...
            if(!@class_exists($value, true)) {
                throw new Mgrid_Grid_Source_Doctrine2_Exception('Entity with name ' . $value . ' does not exist.');
            }
            ...
            throw new Mgrid_Grid_Source_Doctrine2_Exception('Parameter must be an instance of \Doctrine\ORM\EntityRepository or \Doctrine\ORM\QueryBuilder.');
        } else {
            $qb = $value;
        }
    ...
    {
        throw new Mgrid_Grid_Source_Doctrine2_Exception("Fulltext searching is currently not supported by the Doctrine2 source.");
    }
    ...
    public function buildFields()
    {
        if(empty($fields)) {
            ...
            $event = new Mgrid_Grid_Event('source.build_fields', $this, array('fields' => &$returnFields));
     ...
     * to be used when loading the Mgrid_Grid_Form
     *
     * @param array $cols
     * @param array $info
     * @return array
                ...
                foreach($assoc as $field) {
                    if(is_object($field)) {
                        ...
                        if(!method_exists($field, $method)) {
                            throw new Mgrid_Grid_Source_Doctrine2_Exception('No getter method for the primary field found (used name: ' . $method . ').');
                        }
    ...
    {
        throw new Mgrid_Grid_Source_Doctrine2_Exception('Not yet implemented.');
    }
    ...
    /**
     * @todo Implement
     * @see library/Mgrid/Grid/Source/Mgrid_Grid_Source_SourceInterface::getValuesForFiltersFromTable()
     */
     ...
     *
     * @return Mgrid_Grid_Source_Doctrine
     */
    ...
    private function _getModelFromColumn($column)
    {
        if(!is_string($column)) {
            $type = gettype($column);
            require_once 'Mgrid/Grid/Source/Doctrine/Exception.php';
            throw new Mgrid_Grid_Source_Doctrine2_Exception('The $column param needs to be a string, ' . $type . ' provided');
        }
    ...
    private function _getModelFromAlias($alias)
    {
        ...
        if(is_null($field)) {
            throw new Mgrid_Grid_Source_Doctrine2_Exception("No field found.");
        }
        ...
        throw new Mgrid_Grid_Source_Doctrine2_Exception("No model found.");
    }
